...
library(readxl)
aula7 <- read_excel("~/Mestrado (1-4)/LCE5703-Estatística Experimental-2S-2021/aula7.xlsx")
View(aula7)
names(aula7)
head(aula7,10)
```
...
library(stats)
n = with(aula7, tapply(Gen, Rend, length))
soma = with(aula7, tapply(Gen, Rend, sum))
media = with(aula7, tapply(Gen, Rend, mean))
variancia = with(aula7, tapply(Gen, Rend, var))
```

### Criando uma função para calcular a Amplitude  
```{r functions}
f1 = function(x) max(x) - min(x)
amplitude = with(aula7, tapply(Gen, Rend, f1))
...
library(stats)
aula7.anova <- aov(aula7$Gen~aula7$Rend)
summary.aov(aula7.anova)
```
